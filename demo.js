// Manual demo. Run `npm run build` first, then `node demo.js`.
import {
  init,
  getAllCountries,
  getStatesOfCountry,
  getCountryName,
  getState,
  getStateName,
  getAvailableStateLocales,
  hasLocalizedStates,
  createLocalizedData,
  normalizeLocale,
  parseLocale,
} from "./dist/index.js";

init("fr-FR"); // site-wide default: calls without a locale use French

const countries = getAllCountries();
console.log(`${countries.length} countries, site default fr-FR:`);
for (const c of countries.slice(0, 5)) console.log(`  ${c.code}  ${c.name}`);

const es = createLocalizedData("es-ES");
console.log(`\nInstance bound to ${es.locale}: US -> ${es.getCountryName("US")}`);

console.log("\nCountry names per locale (per-call API):");
for (const locale of ["es-ES", "de-DE", "zh-CN", "ar-SA", "ru-RU"]) {
  console.log(`  US in ${locale}: ${getCountryName("US", locale)}`);
}

console.log("\nUS states in Spanish:");
for (const s of getStatesOfCountry("US", "es-ES").slice(0, 5)) {
  console.log(`  ${s.code}  ${s.name}`);
}

console.log("\nSingle lookups (bare or full subdivision codes):");
console.log(`  getState("US", "CA", "ru") ->`, getState("US", "CA", "ru"));
console.log(`  getStateName("DE", "DE-BY", "fr") -> ${getStateName("DE", "DE-BY", "fr")}`);

console.log("\nLocale utilities:");
console.log(`  normalizeLocale("en-us") -> ${normalizeLocale("en-us")}`);
console.log(`  parseLocale("zh-Hans-CN") ->`, parseLocale("zh-Hans-CN"));
console.log(`  hasLocalizedStates("US", "es-ES") -> ${hasLocalizedStates("US", "es-ES")}`);
console.log(`  getAvailableStateLocales("US") -> ${getAvailableStateLocales("US").join(", ")}`);
console.log(`  getAllCountries() unchanged: ${getAllCountries().length} countries (English)`);
